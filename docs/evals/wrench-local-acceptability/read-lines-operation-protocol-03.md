...
- Reuse the six case definitions in
  `tools/measure_read_lines_acceptability.py` at the run's pinned repository
  revision. Two successful cases use the same exact prompt against two files
