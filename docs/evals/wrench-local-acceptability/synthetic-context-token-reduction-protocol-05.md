# Synthetic M3 context-token reduction protocol 05

Status: **frozen for one offline run**. Supersedes protocol 04, whose run
loaded the pinned tokenizer and completed case calculations but failed while
formatting the budget-boundary summary; no counts or receipt were persisted.
Protocols 02 and 03 also stopped without producing receipts. Earlier
explicit-operation proxy protocols remain superseded and are not execution
authority.

Job ID: `W2-SYN-M3-CTX-REDUCTION-20260925-04`  
Nonce: `SYNCTX06-CA42`  
Runner: `tools/measure_synthetic_context_token_reduction.py`  
Runner SHA-256: `cd6cda317c638f5ba621f601e539bb8ac44eeb1212d6ed6610a7ac2e886e26cb`
Frozen one-shot output: `C:\wrench-slm-data\artifacts\wrench-local-acceptability\synthetic-context-m3-reduction-04.json`
...
- Use the already held reservation `W2-SYN-M3-CTX-REDUCTION-20260925-04` for
...
- Frozen command:
  ...
  ```powershell
  C:\wrench-slm-data\envs\wrench-local-synthetic-cp313\Scripts\python.exe tools\measure_synthetic_context_token_reduction.py --output C:\wrench-slm-data\artifacts\wrench-local-acceptability\synthetic-context-m3-reduction-04.json
  ```
