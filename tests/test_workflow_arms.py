...
def test_workflow_scoring_requires_paired_safety_and_savings_gates():
    receipt = evaluate_manifest({"schema": "wrench.workflow-arm-traces.v1", "authorization": "approved_real_workflow", "traces": [_trace(str(index)) for index in range(20)]})
    ...
    unsafe = evaluate_manifest({"schema": "wrench.workflow-arm-traces.v1", "authorization": "approved_real_workflow", "traces": [_trace("unsafe", mutation=True)]})
