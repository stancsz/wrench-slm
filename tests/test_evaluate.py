...
def test_baseline_evaluation_on_held_out_is_bounded():
    summary = evaluate_baseline("data/archive/baseline/held_out.jsonl", limit=200)
...
def test_canary_reports_local_speedup():
    canary = run_canary(ProductionDataBaseline(), "data/archive/baseline/held_out.jsonl", limit=200)
...
def test_held_out_records_have_required_fields():
    path = Path("data/archive/baseline/held_out.jsonl")
