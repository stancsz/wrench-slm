...
_SCHEMA = "wrench.e0.baseline-accounting.v2"
...
class E0BaselineAccountingReceipt:
    ...
    context_selected_candidate_order_sha256: str
    context_selected_candidate_count: None
    context_omitted_candidate_count: None
    synthetic_serializer_id: str
    synthetic_envelope_sha256: str
    synthetic_envelope_bytes: int
    synthetic_envelope_chars: int
    synthetic_tokenizer_id: str
    synthetic_token_count: int
    lowered_request_body_sha256: str
    lowered_request_body_bytes: int
...
def build_e0_baseline_accounting_receipt(
    ...
    post_insertion_event: Mapping[str, object],
    ...
    if (
        ...
        or type(lowered_request) is not LoweredRequest
        or not isinstance(post_insertion_event, Mapping)
...
def verify_e0_baseline_accounting_receipt(receipt: E0BaselineAccountingReceipt) -> bool:
    """Verify the content-free digest and explicit status/accounting closure."""
    ...
    try:
        ...
        if (
            ...
            or receipt.context_selected_candidate_count is not None
            or receipt.context_omitted_candidate_count is not None
            or receipt.synthetic_serializer_id != SYNTHETIC_SERIALIZER_ID
            or receipt.synthetic_tokenizer_id != SYNTHETIC_TOKENIZER_ID
            or receipt.terminal_outcome not in {end.value for end in StreamEnd}
            ...
            or dict(receipt.inventory_status_counts).get(CoverageFileStatus.INDEXED.value, 0) != receipt.inventory_entry_count
            ...
            or receipt.accounted_snapshot_manifest_entries != receipt.inventory_entry_count
            or receipt.unaccounted_snapshot_manifest_entries != 0
            or receipt.exact_read_attempts != receipt.inventory_entry_count
            or receipt.exact_read_successes != receipt.inventory_entry_count
            ...
            or not _is_int(receipt.context_candidate_count)
            or receipt.context_candidate_count < 1
            or not _is_int(receipt.exact_read_returned_bytes)
            or not 0 <= receipt.exact_read_returned_bytes <= 4 * 1024 * 1024
            or not _is_int(receipt.synthetic_envelope_bytes)
            or receipt.synthetic_envelope_bytes < 1
            or not _is_int(receipt.synthetic_envelope_chars)
            or receipt.synthetic_envelope_chars < 1
            or not _is_int(receipt.synthetic_token_count)
            ...
            or receipt.synthetic_token_count != receipt.synthetic_envelope_chars
            or not _is_int(receipt.lowered_request_body_bytes)
            or not 1 <= receipt.lowered_request_body_bytes <= MAX_REQUEST_BYTES
            ...
            or any(not _DIGEST.fullmatch(value) for value in (
                ...
                receipt.preparation_sha256, receipt.insertion_receipt_sha256,
                receipt.semantic_projection_sha256, receipt.context_selected_candidate_order_sha256,
                receipt.synthetic_envelope_sha256,
                receipt.lowered_request_body_sha256, receipt.composition_receipt_sha256,
            ))
            or (receipt.route_preparation_sha256 is not None and not _DIGEST.fullmatch(receipt.route_preparation_sha256))
        ):
            return False
        return _sha256(_canonical(_payload(receipt))) == receipt.receipt_sha256
    except (AttributeError, TypeError, ValueError, UnicodeError, RecursionError):
        return False
