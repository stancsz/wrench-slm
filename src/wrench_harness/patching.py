...
def add_patch_retry_instruction(messages: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    return add_bounded_repair_instruction(messages, "patch_output_malformed")


def add_bounded_repair_instruction(messages: list[dict[str, str]], reason: str) -> list[dict[str, str]]:
    """Make one bounded corrective retry explicit after a format failure.

    ``reason`` is generated by the local verifier and is deliberately included
    as a small diagnostic, not as user-controlled prompt text. The retry is
    still subject to the same parser, authority checks, and TTC gate.
    """
    ...
    for message in retried:
        if message.get("role") in {"system", "developer"}:
            message["content"] = str(message.get("content", "")) + (
                f" The previous Wrench response failed local format validation ({reason}). "
                "Retry once and emit one complete JSON object only. Do not add markdown, "
                "prose, or unsupported actions."
            )
            break
    return retried
