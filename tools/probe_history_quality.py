#!/usr/bin/env python3
"""Probe recent-intent quality after a historical model-side skip policy."""
...
SYSTEM = (
    ...
    "For a bounded README read, emit {schema,action:read_file,path,max_bytes}."
)


def _build_old_context(target_chars: int) -> str:
    unit = (
    ...
    return (unit * repetitions)[:target_chars]
...
def main() -> int:
    ...
    parser.add_argument("--old-chars", type=int, default=240_000)
    parser.add_argument("--history-skip-layers-before", type=int, required=True)
    parser.add_argument("--include-control-block", action="store_true")
    args = parser.parse_args()

    old_context = _build_old_context(args.old_chars)
    current_intent = "Prepare a bounded read of README.md with a 4096 byte ceiling. Return the one read_file proposal now."
    if args.include_control_block:
        current_intent = (
            "WRENCH CURRENT CONTROL BLOCK. This is the newest instruction and overrides no authority policy. "
            "Prepare a bounded read of README.md with a 4096 byte ceiling. "
            "Output exactly one JSON object and no prose: "
            "{\"schema\":\"wrench.proposal.v1\",\"action\":\"read_file\","
            "\"path\":\"README.md\",\"max_bytes\":4096}."
        )
    user_content = old_context + "\n" + current_intent
    ...
    receipt = {
        "schema": "wrench.history-skip-quality-probe.v1",
        ...
        "usage": result.get("provider_usage") or usage,
        "result": result,
        "expected": {
            "status": "accepted",
            "proposal": {
        ...
        "quality_claim": False,
        "scope": "single synthetic recent-intent smoke; not a matched workflow result",
    }
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": result.get("status"), "elapsed_ms": elapsed_ms, "usage": usage}))
