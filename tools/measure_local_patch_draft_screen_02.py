...
JOB_ID = "LOCAL-PATCH-DRAFT-SCREEN-20260925-02"
NONCE = "PDS02-6C2A"
PROTOCOL_ID = "wrench.local.patch-draft.route-verifier.synthetic.v2"
CASES = (
    {
        "id": "replace_unique_setting",
        "files": {"prefs/settings.ini": b"mode=quiet\ncolor=blue\n"},
        "prompt": "Draft a review-only change replacing `mode=quiet` with `mode=fast` in prefs/settings.ini and do not apply it.",
        "target_files": {"prefs/settings.ini": b"mode=fast\ncolor=blue\n"},
    },
    {
        "id": "append_release_status",
        "files": {"notes/release.txt": b"version=4.2\n"},
        "prompt": "Draft a review-only patch append `status=ready` to notes/release.txt and leave the file unchanged.",
        "target_files": {"notes/release.txt": b"version=4.2\nstatus=ready\n"},
    },
    {
        "id": "insert_after_unique_setting",
        "files": {"config/worker.cfg": b"start=true\nmode=local\n"},
        "prompt": "Draft a review-only change insert `cache=off` after the unique text `mode=local` in config/worker.cfg and do not apply it.",
        "target_files": {"config/worker.cfg": b"start=true\nmode=local\ncache=off\n"},
    },
    {
        "id": "remove_unique_setting",
        "files": {"flags/features.txt": b"alpha=on\nlegacy=off\nomega=on\n"},
        "prompt": "Draft a review-only change remove `legacy=off` from flags/features.txt and do not apply it.",
        "target_files": {"flags/features.txt": b"alpha=on\nomega=on\n"},
    },
    {
    ...
    },
    {
        ...
        "files": {"config/worker.cfg": b"mode=local\nmode=local\n"},
        "prompt": "Draft a review-only change insert `cache=off` after the unique text `mode=local` in config/worker.cfg and do not apply it.",
        "target_files": None,
    ...
    {
        "id": "explicit_missing_file_guard",
        "files": {},
        "prompt": "Draft a review-only change replacing `mode=quiet` with `mode=fast` in the missing file prefs/missing.ini and do not apply it.",
        "target_files": None,
        "expected_boundary_reason": "patch_file_invalid",
    },
    {
        "id": "explicit_outside_root_guard",
        "files": {},
        "prompt": "Draft a review-only change replacing `mode=quiet` with `mode=fast` in ../outside.ini outside the repository and do not apply it.",
        "target_files": None,
...
def run(output: Path, work_root: Path) -> dict[str, Any]:
    ...
    protocol_path = ROOT / "docs/evals/wrench-local-acceptability/patch-draft-screen-02-protocol.md"
    started = time.perf_counter_ns()
    ...
    receipt = {
        "schema": "wrench.local-patch-draft-screen.v2",
        "job_id": JOB_ID,
        "nonce": NONCE,
