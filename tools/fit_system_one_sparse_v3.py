"""Fit the fixed sparse Qwen readout on audited v3 unsealed contrasts.
...
def main(args):
    ...
    if (len(old["labels"]) < len(old_rows) * 2 or len(new_rows) != len(new["labels"])
    ...
    qwen = np.concatenate([old["features"][1:len(old_rows)*2:2].numpy(), new["features"].numpy()])
    if not np.array_equal(labels[:len(old_rows)], old["labels"][1:len(old_rows)*2:2].numpy()) or not np.array_equal(
    ...
    receipt = {"schema": "wrench.system-one-sparse-v3-fit.v1", "status": "CALIBRATED_HEAD_READY",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--new-features", type=Path, default=Path("artifacts/system-one-readiness/policy-contrasts-v3/features.pt"))
    parser.add_argument("--output", type=Path, default=Path("artifacts/system-one-readiness/sparse-v3"))
