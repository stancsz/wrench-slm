...
def _build_prompt(path: Path | None, target_tokens: int, tokenizer_path: Path | None) -> tuple[str, str, int | None]:
    if path is not None:
        return path.read_text(encoding="utf-8"), "prompt_file", None
    if target_tokens <= 0:
...
def main() -> int:
    ...
    prompt, build_method, estimated_content_tokens = _build_prompt(args.prompt_file, args.target_tokens, args.tokenizer)
