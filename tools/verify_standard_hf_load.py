...
def verify(model_dir: Path) -> dict[str, Any]:
    import transformers
...
def main() -> int:
    ...
    receipt = verify(args.model_dir.resolve())
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"status": receipt["status"], "transformers_version": receipt["transformers_version"]}))
    return 0
