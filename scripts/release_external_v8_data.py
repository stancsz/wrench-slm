"""Generate fresh v8 external evaluation sets.
...
def context_for(dataset, kind, language, index, path, folder, service):
    ...
    decoy_service = 'svc-v8-decoy-' + token[:5]
    decoy_path = 'archive-v8/' + token[5:11] + '.json'
    resources = [
...
def make_row(dataset, split, kind, language, index):
    ...
    service = 'svc-v8-' + token[:8]
    if language == 'zh':
        path = f'配置 v8/{service} 的记录 {token[8:14]}.json'
        folder = f'检索区 v8/{token[14:20]}'
    else:
        path = f'service records v8/{service} note {token[8:14]}.json'
        folder = f'search area v8/{token[14:20]}'
    marker = ['BUILD-V8=' + token[20:28], 'retry?window=' + token[20:26], '状态码=v8-' + token[20:26]][index % 3]
    ...
    content = f'v8-review-{language}-{token[32:40]}\nregion-note={token[40:46]}\n'
    ...
    fixture['files'][path] = json.dumps({'service': service, 'region': 'region-v8-' + token[46:54], 'replicas': 2 + int(token[54], 16) % 7}, ensure_ascii=False)
    fixture['files'][decoy_path] = json.dumps({'service': decoy_service, 'region': 'decoy-v8-' + token[55:63], 'replicas': 1}, ensure_ascii=False)
    call = {'tool': 'read_file', 'args': {'path': path}}
    expected = json.loads(fixture['files'][path])['region']
    if kind == 'lines':
        lines = [f'v8-{language}-{kind}-{token[:6]}-{n:03d}' for n in range(1, 32)]
        fixture['files'][path] = '\n'.join(lines) + '\n'
        ...
        fixture['git'] = True
        fixture['dirty'] = [f'changed-v8-{token[:7]}.txt', f'new-v8-{token[7:14]}.txt']
        call = {'tool': 'exec_command', 'args': {'cmd': 'git status --short'}}
        expected = sorted(fixture['dirty'])
    elif kind == 'git_log':
        fixture['git'] = True
        fixture['subject'] = f'V8 refresh {token[:12]}'
    ...
    if kind == 'invalid_range':
        if language == 'en':
            prompt = f'This line request is invalid. Do not read any file and do not call read_file. Return ROUTER_FALLBACK. Target path: {path}. Requested bounds: {values["start"]} through {values["end"]}.'
        else:
            prompt = f'这个行范围无效。不要读取任何文件，也不要调用 read_file。请返回 ROUTER_FALLBACK。目标路径：{path}。请求范围：第 {values["start"]} 到第 {values["end"]} 行。'
    if kind == 'search':
        if language == 'en':
            prompt += f' Search marker: {quoted(marker)}. Search folder: {quoted(folder)}.'
        else:
            prompt += f' 搜索标记：{quoted(marker)}。搜索目录：{quoted(folder)}。'
    if kind in ('lines', 'invalid_range'):
        ...
        if kind == 'invalid_range':
            prompt += ' Return ROUTER_FALLBACK and do not call a tool.' if language == 'en' else ' 请返回 ROUTER_FALLBACK，不要调用工具。'
    elif kind == 'config':
        if language == 'en':
    ...
    return {
        ...
        'seed_id': identifier,
        'family_id': family,
        'source': 'fresh_v8_authored_external_scenario',
        'kind': kind,
        'language': language,
        'prompt': prompt,
        'context': context,
...
def write_dataset(dataset, output, counts):
    ...
    manifest = {
        'version': dataset,
        'source': 'Fresh authored V8 data. No model predictions or scored V4 rows imported.',
