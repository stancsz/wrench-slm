"""Generate fresh v7 external evaluation sets.
...
def context_for(dataset, kind, language, index, path, folder, service):
    ...
    decoy_service = 'svc-v7-decoy-' + token[:5]
    decoy_path = 'archive-v7/' + token[5:11] + '.json'
    resources = [
...
def make_row(dataset, split, kind, language, index):
    ...
    service = 'svc-v7-' + token[:8]
    if language == 'zh':
        path = f'配置 v7/{service} 的记录 {token[8:14]}.json'
        folder = f'检索区 v7/{token[14:20]}'
    else:
        path = f'service records v7/{service} note {token[8:14]}.json'
        folder = f'search area v7/{token[14:20]}'
    marker = ['BUILD-V7=' + token[20:28], 'retry?window=' + token[20:26], '状态码=v7-' + token[20:26]][index % 3]
    ...
    content = f'v7-review-{language}-{token[32:40]}\nregion-note={token[40:46]}\n'
    ...
    fixture['files'][path] = json.dumps({'service': service, 'region': 'region-v7-' + token[46:54], 'replicas': 2 + int(token[54], 16) % 7}, ensure_ascii=False)
    fixture['files'][decoy_path] = json.dumps({'service': decoy_service, 'region': 'decoy-v7-' + token[55:63], 'replicas': 1}, ensure_ascii=False)
    call = {'tool': 'read_file', 'args': {'path': path}}
    expected = json.loads(fixture['files'][path])['region']
    if kind == 'lines':
        lines = [f'v7-{language}-{kind}-{token[:6]}-{n:03d}' for n in range(1, 32)]
        fixture['files'][path] = '\n'.join(lines) + '\n'
        ...
        fixture['git'] = True
        fixture['dirty'] = [f'changed-v7-{token[:7]}.txt', f'new-v7-{token[7:14]}.txt']
        call = {'tool': 'exec_command', 'args': {'cmd': 'git status --short'}}
        expected = sorted(fixture['dirty'])
    elif kind == 'git_log':
        fixture['git'] = True
        fixture['subject'] = f'V7 refresh {token[:12]}'
    ...
    return {
        ...
        'seed_id': identifier,
        'family_id': family,
        'source': 'fresh_v7_authored_external_scenario',
        'kind': kind,
        'language': language,
        'prompt': prompt,
        'context': context,
...
def write_dataset(dataset, output, counts):
    ...
    manifest = {
        'version': dataset,
        'source': 'Fresh authored V7 data. No model predictions or scored V4 rows imported.',
...
def main():
    challenge = write_dataset('independent-challenge-v7', 'artifacts/archive/data/pilots/independent-challenge-v7', 3)
    sealed = write_dataset('release-authoring-v7', 'artifacts/archive/data/pilots/release-authoring-v7', 20)
