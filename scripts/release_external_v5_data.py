"""Generate fresh v5 external evaluation sets.
...
def context_for(dataset, kind, language, index, path, folder, service):
    ...
    decoy_service = 'svc-v5-decoy-' + token[:5]
    decoy_path = 'archive-v5/' + token[5:11] + '.json'
    resources = [
...
def make_row(dataset, split, kind, language, index):
    ...
    service = 'svc-v5-' + token[:8]
    if language == 'zh':
        path = f'配置 v5/{service} 的记录 {token[8:14]}.json'
        folder = f'检索区 v5/{token[14:20]}'
    else:
        path = f'service records v5/{service} note {token[8:14]}.json'
        folder = f'search area v5/{token[14:20]}'
    marker = ['BUILD-V5=' + token[20:28], 'retry?window=' + token[20:26], '状态码=v5-' + token[20:26]][index % 3]
    ...
    content = f'v5-review-{language}-{token[32:40]}\nregion-note={token[40:46]}\n'
    ...
    fixture['files'][path] = json.dumps({'service': service, 'region': 'region-v5-' + token[46:54], 'replicas': 2 + int(token[54], 16) % 7}, ensure_ascii=False)
    fixture['files'][decoy_path] = json.dumps({'service': decoy_service, 'region': 'decoy-v5-' + token[55:63], 'replicas': 1}, ensure_ascii=False)
    call = {'tool': 'read_file', 'args': {'path': path}}
    expected = json.loads(fixture['files'][path])['region']
    if kind == 'lines':
        lines = [f'v5-{language}-{kind}-{token[:6]}-{n:03d}' for n in range(1, 32)]
        fixture['files'][path] = '\n'.join(lines) + '\n'
        ...
        call = {'tool': 'exec_command', 'args': {'cmd': f'rg -l --fixed-strings -- {shlex.quote(marker)} {shlex.quote(folder)}'}}
        expected = [f'{folder}/target-one.txt', f'{folder}/target-two.txt']
    elif kind == 'git_status':
        fixture['git'] = True
        fixture['dirty'] = [f'changed-v5-{token[:7]}.txt', f'new-v5-{token[7:14]}.txt']
        call = {'tool': 'exec_command', 'args': {'cmd': 'git status --short'}}
        expected = sorted(fixture['dirty'])
    elif kind == 'git_log':
        fixture['git'] = True
        fixture['subject'] = f'V5 refresh {token[:12]}'
    ...
    return {
        ...
        'seed_id': identifier,
        'family_id': family,
        'source': 'fresh_v5_authored_external_scenario',
        'kind': kind,
        'language': language,
        'prompt': prompt,
        'context': context,
...
def write_dataset(dataset, output, counts):
    ...
    manifest = {
        'version': dataset,
        'source': 'Fresh authored V5 data. No model predictions or scored V4 rows imported.',
...
def main():
    challenge = write_dataset('independent-challenge-v5', 'artifacts/archive/data/pilots/independent-challenge-v5', 3)
    sealed = write_dataset('release-authoring-v5', 'artifacts/archive/data/pilots/release-authoring-v5', 20)
